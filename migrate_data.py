...
try:
    ...
    has_tqdm = False
    print("tqdm package not found. Progress bars will be disabled.")
...
def migrate_table_data(sqlite_conn, pg_conn, sqlite_table, pg_table):
    ...
    try:
        ...
        batch_size = 1000
        if has_tqdm:
            progress_bar = tqdm(total=total_rows, desc=f"Migrating {sqlite_table}", unit="rows")
    ...
    try:
        ...
        for sqlite_table in sqlite_tables:
            if sqlite_table in TABLE_MAPPING:
                ...
                # Create indices
                create_indices(pg_conn, sqlite_table)
